                ...
                <li className={menuOpen[0] == index ? "js-opened" : ""} key={index}>
                    <a
                        ...
                        className={`mn-has-sub ${
                            item.subMenu.some((e1) => e1.links.some((e2) => e2.href.split("/")[1] == pathname.split("/")[1])) ? "active" : ""
                        }`}
                    >
                    ...
                    <ul className={`mn-sub mn-has-multi ${menuOpen[0] == index ? "mobile-sub-active" : ""} `}>
                        {item.subMenu.map((subItem, subIndex) => (
                            <li className='mn-sub-multi' key={subIndex}>
                                {subItem.title && <span className='mn-group-title'>{subItem.title}</span>}
                                <ul>
            ...
            {links.slice(3, 5).map((item, index) => (
                <li className={menuOpen[0] == index + 3 ? "js-opened" : ""} key={index}>
                    <a
                        ...
                        href='#'
                        className={`mn-has-sub ${
                            item.subItems.some((e1) => e1.links.some((e2) => e2.href.split("/")[1] == pathname.split("/")[1]))
                                ? "active"
                    ...
                    {item.subItems && (
                        <ul className={`mn-sub  ${menuOpen[0] == index + 3 ? "mobile-sub-active" : ""} `}>
                            {item.subItems.map((subItem, subIndex) => (
                                <li className={menuOpen[1] == subIndex ? "js-opened" : ""} key={subIndex}>
                                    <a
                                        href='#'
                                        onClick={() => toggleParent2(subIndex)}
                                        className={`mn-has-sub ${
                                            subItem.links.some((e2) => e2.href.split("/")[1] == pathname.split("/")[1]) ? "active" : ""
                                        }`}
                                    >
                                        {subItem.title} <i className='mi-chevron-right right' />
                                    </a>
                                    {subItem.links && (
                                        <ul className={`mn-sub  ${menuOpen[1] == subIndex ? "mobile-sub-active" : ""} `}>
                                            {subItem.links.map((link, linkIndex) => (
                                                <li key={linkIndex}>
                                                    <Link
                            ...
                            {!index && (
                                <>
                                    {item.subItems[0].links[0].href.includes("-dark") ? (
                                        <li>
                                            <Link
                                                className={pathname.split("/")[1].includes("main-portfolio-lazyload") ? "active" : ""}
